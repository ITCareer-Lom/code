...
namespace TheGameMVC
{
    public class StartUp
    {
        ...
        static public Map DbToMap(int mapNo = 0)
        {
            ...
            using (var gameContext = new GameContext())
            {
                maps = gameContext.Maps.Include(map => map.Heroes).ToList();
                foreach (var map in maps) {
                    map.Levels = gameContext.Levels
                        .Include(level => level.Helpers)
                            .ThenInclude(helper => helper.Items)
                        .Include(level => level.Enemies)
                            .ThenInclude(enemy => enemy.Items).ToList();
                }               
            }
            return maps[mapNo]; 
        }
